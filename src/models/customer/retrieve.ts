...
export async function retrieveCustomer(name: string, id?: number) {
  const [dataSourceCustomer, templateCustomer] = await Promise.all([getStorage("DATA_SOURCE_CUSTOMER"), getStorage("TEMPLATE_CUSTOMER"),])

  if (dataSourceCustomer && templateCustomer) {
