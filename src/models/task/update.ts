...
import { createTaskQueue, updateTaskAssigneeQueue, updateTaskCustomerQueue } from "../../queues";
  ...
  if (taskData) {
    ...
    if (updated_at.diff(taskData.updated_at) > 0) {
      ...
      await updateTaskCustomerQueue.add(
        ...
        { notion_id, project: data.customer, customer_id: data.customer_id },
      ...
      if (data.people && data.people !== taskData.data.people) {
        await updateTaskAssigneeQueue.add(
          ...
          { notion_id, assignee: data.people },
          CONTROLLED_RETRY_OPTIONS,
        );
      }
    }
